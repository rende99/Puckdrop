package org.PuckMiddle.controller;
...
import org.PuckMiddle.manager.RequestManager;
...
public class RequestController {
	...
	public ResponseEntity<Void> changePassword(@RequestBody PasswordChange passwordObject) throws IOException {
		...
		requestManager.getUserService().changePassword(passwordObject.getOldPassword(),
	...
	public String getTeamSchedule(@PathVariable("id") String id) throws IOException {
		...
		return requestManager.getApiService().consumeEndpoint("/schedule?teamId=",
				id + "&startDate=" + now + "&endDate=" + end);
	}

	@RequestMapping(value = "/schedule/{sd}/{ed}", method = RequestMethod.GET)
	public String getScheduleBetweenDates(@PathVariable("sd") String sd, @PathVariable("ed") String ed)
			throws IOException {
